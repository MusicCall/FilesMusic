...
async def _(event):
    ...
    if event.pattern_match.group(2):
        ...
        await eor(event, "** ⌔∮ يجب عليك كتابة اسم الاغنيه لكي اقوم بالبحث عنها 🖤،** `")
        return
    ...
    try:
        ...
        pass
    stderr = (await _wizardutils.runcmd(song_cmd))[1]
    if stderr:
        return await icse.edit(f"** ⌔∮ خطا :** `{stderr}`")
    icsn, stderr = (await _wizardutils.runcmd(name_cmd))[:2]
    if stderr:
        return await icse.edit(f"** ⌔∮ خطا :** `{stderr}`")
...
async def _(event):
    ...
    stderr = (await _wizardutils.runcmd(video_cmd))[1]
    if stderr:
        return await icse.edit(f"** ⌔∮ خطا :** `{stderr}`")
    icsn, stderr = (await _wizardutils.runcmd(name_cmd))[:2]
    if stderr:
...
@wizardbot.on(admin_cmd(pattern="اغنيه (.*)"))
@wizardbot.on(sudo_cmd(pattern="اغنيه (.*)", allow_sudo=True))
async def wizardongfetcer(event):
    if event.fwd_from:
        return
